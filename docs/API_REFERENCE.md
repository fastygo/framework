...
### `internal/application/docs/handler.go`
...
- `type DocsListQueryResult struct { Pages []DocsListItem }`
  - `type DocsListItem struct { Slug, Title string }`
- `type DocsPageQuery struct { Slug string }`
- `type DocsPageResult struct { Slug, Title, HTML string }`
- `type DocsPageQueryHandler struct`
  ...
  - `type DocsPageRender struct { Title, HTML string }`
- `type DocsListQueryHandler struct`
  - `func (h DocsListQueryHandler) Handle(_ context.Context, _ DocsListQuery) (DocsListQueryResult, error)`

### `internal/application/docs/registry.go`
...
### `internal/infra/features/docs/module.go`
...
- `func (m *Module) SetNavItems(items []app.NavItem)`
- `func (m *Module) Routes(mux *http.ServeMux)`
- `func (m *Module) handleDocsIndex(w http.ResponseWriter, r *http.Request)`
  - Renders `/docs` index from preloaded registry.
- `func (m *Module) handleDocsPage(w http.ResponseWriter, r *http.Request)`
  - Renders `/docs/{slug}` from cached html.

### `internal/site/docs/content/embed.go`

`package content`

#### Globals

- `var FS embed.FS` with `//go:embed *.md`

### `internal/site/docs/web/views/models.go`
...
- `type DocsListItem struct { Slug, Title string }`
- `type DocsLayoutData struct { Title, BrandName, Active string; NavItems []app.NavItem; ThemeToggle web.ThemeToggleData }`
- `type DocsPageData struct { Title, HTMLContent string }`
- `type DocsIndexData struct { Pages []DocsListItem }`

### `internal/site/docs/web/views/layout.templ`

`package views`

#### Functions

- `func DocsLayout(data DocsLayoutData, headExtra templ.Component, body templ.Component) templ.Component`

### `internal/site/docs/web/views/page.templ`
...
### `internal/site/docs/web/views/index.templ`

`package views`

#### Functions

- `func DocsIndex(data DocsIndexData) templ.Component`
...
- Loads app config with docs defaults (`APP_BIND: 127.0.0.1:8081`, `APP_STATIC_DIR: internal/site/docs/web/static`).
- Builds markdown pre-render cache via `docs.NewDocsPageQueryHandler`.
- Registers docs query handlers and doc feature.
- Runs docs server with the same middleware and lifecycle behavior as the main site.
